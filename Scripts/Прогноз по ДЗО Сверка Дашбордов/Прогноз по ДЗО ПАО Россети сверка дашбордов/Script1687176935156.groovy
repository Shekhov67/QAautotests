...
def Test(def run, def todaysDate, def widget) {
    ...
    WebUI.scrollToElement(findTestObject('Прогноз по ДЗО стресс сценарий/Июль'), 30)

    WebUI.scrollToElement(findTestObject('Прогноз по ДЗО/скролл'), 30)

    WebUI.click(findTestObject('Прогноз по ДЗО стресс сценарий/Июль'), FailureHandling.CONTINUE_ON_FAILURE)
